 ...
 */
public class universalScoring implements Initializable {
	...
	private void makeDelays(String start, String end) {
		...
		arry.add(start);
		arry.add(end);
		DBmessage dbm = new DBmessage(MessageType.makeDelays, arry);
		try {
	...
	private void makePerformenct(String start, String end) {
		...
		arry.add(start);
		arry.add(end);
		DBmessage dbm = new DBmessage(MessageType.makePerformenct, arry);
		try {
			MainAllControllers.sendToAbsServer(dbm);
		} catch (IOException we) {
		}
	 ...
	 * Make active su clo.
	 */
	public static ArrayList<Object> makeActiveSuClo(String start, String end) {
		ArrayList<Object> arry = new ArrayList<Object>();
		arry.add(start);
		arry.add(end);
		ArrayList<Object> send;
		send= universalScoring.getActiveSuClo(arry);
		return send;
	}
	/**
	...
	public void setActiveSuClo(ArrayList<Object> send) {
		...
		devf = StandardDeviation(failure);
		devs =  StandardDeviation(success);
		devsus = StandardDeviation(susppend);
	...
	public float StandardDeviation(int[] arr) {
		...
		avrg/=arr.length;
		for (int i=0;i<arr.length;i++) 
			dev += (float) Math.pow(arr[i] - avrg, 2);
		dev = (float) Math.sqrt(dev /arr.length);
		return dev;
	}
